...
// First step is to extract the list of addresses generated by the smart contract
contract('Dex', (accounts) => {
  let dai, bat, rep, zrx, dex;
  // Extract to addresses specifically
  ...
  beforeEach(async() => {
    ...
    //define the amount of tokens to allocate to addresses
    const amount = web3.utils.toWei('1000');
    //accepts address of token and trader
    const seedTokenBalance = async (token, trader) => {
      //allocates token to address
      await token.faucet(trader, amount)
      //trader will approve amount
      await token.approve(
        dex.address, 
        amount, 
        {from: trader}
    ...
    //loop through all tokens and call the seedTokenBalance function for trader 1
    await Promise.all(
      [dai, bat, rep, zrx].map(
        token => seedTokenBalance(token, trader1) 
      )
    );
    //loop through all tokens and call the seedTokenBalance function for trader 2
    await Promise.all(
      [dai, bat, rep, zrx].map(
  ...
  it('should NOT deposit tokens if token does not exist', async () => {
    await expectRevert(
      dex.deposit(
        //specify token that doesn't exist
  ...
  it('should withdraw tokens', async () => {
    ...
    //balance of dex should be zero
    assert(balanceDex.isZero());
    //balance of wei should be 1000, the initial balance
