...
//Add the exchange addresses through the constructor during deploy
//This is where you could change some exchanges if they were also
//uniswap clones
contract FlashSwap {
...
//Here we take the repay amount and the endRouter address to fill in the route.
//These can both be calculated on contract but will cost you every time you run it
//to get profits
 function pancakeCall(address sender, uint amount0, uint amount1, bytes calldata data) external {
       ...
       ( address endRouter, uint repay) = abi.decode(data, (address, uint));
        ...
        if (amountToken > 0) {
            ...
            //Withdrawing the funds based on route :
            ...
            //Here we are transferring the repay amount to the pair contract first.
            //Then if anything is left over it is sent back to our predefined 
            //beneficiary address.
            if(amountReceived[1] - repay > 0) {
